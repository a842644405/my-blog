...
**协议（Protocol），**定义的规则集，确保网络能够通信。

协议：各方协商一致后确定的规则，用于规范网络中的数据交换过程。遵循相同的协议，不同设备和系统间就能顺利地进行数据交互。
通用协议：全球广泛接受和使用，例如TCP/IP、HTTP等，构成了互联网通信的基础架构，确保了跨地域、跨平台的信息交流。
私有协议：某些组织或企业会根据自身需求设计内部使用的协议，即私有协议。这类协议通常用于特定场景下的优化或安全增强。
...
1、物理层
解决信号转换的问题，基本都是数字信号，二进制数据控制的电路信号、电磁信号：电信号、
...
SYN_SENT                                          SYN_RCVD
  ...
  │    （同意建连，确认x，告诉对方我的初始序号y）      │
  ▼                                                  ▼
ESTABLISHED                                      （仍 SYN_RCVD）
...
- ① SYN 占一个序号，所以客户端下一个报文 seq=x+1 

- ② 服务器把“确认客户端 SYN”和“发自己 SYN”合在一个包，省一次往返，所以变三次而不是四次 

- ③ 纯 ACK 可不带数据、不消耗序号（带了数据才消耗）

- 目的：双方互相确认“能发能收”+ 同步初始序列号 ISN，防旧 SYN 建连
...
FIN_WAIT_1                                        CLOSE_WAIT
  │◀─② ACK=1, seq=v, ack=u+1 ─────────────────────│
  │    （收到FIN，先确认；服务器还可继续发数据）        │
  ▼                                                  │
FIN_WAIT_2                                         │  ……服务器把剩余数据发完……
  │                                                 │
...
TIME_WAIT                                         LAST_ACK
  ...
  │    （最终确认）                                  │
  ▼                                                  ▼
TIME_WAIT ──等待 2MSL──▶ CLOSED                   CLOSED

状态对照：

- 客户端：FIN_WAIT_1 →（收到ACK）FIN_WAIT_2 →（收到对端FIN）TIME_WAIT →（2MSL）CLOSED
- 服务器：CLOSE_WAIT（收到对端FIN后） → LAST_ACK（发自己FIN后） → CLOSED 
- 2MSL：确保最后ACK丢了对端重发FIN能再回；同时让旧报文在网络中失效。Linux 常见配 30s/60s，RFC 传统按 2 分钟讲 

**为什么是“三次建、四次断”**

- 建连：服务器可以把“确认你”的 ACK 和“我也同步”的 SYN 合并 → 3 次
- 断连：收到对端 FIN 只代表“对端不发了”，本端可能还有数据要发，所以 ACK 和自己的 FIN 通常分开 → 4 次 
  例外：如果服务器收到 FIN 时已经没数据可发，ACK+FIN 可合并，看起来像“三次挥手” 
